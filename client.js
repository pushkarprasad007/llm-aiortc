    ...
    return state.pc.createOffer().then((offer) => {
        ...
        return new Promise((resolve) => {
        	...
        	console.log("ICE Gathering Complete");
                ...
                const checkState = () => {
                    if (state.pc.iceGatheringState === 'complete') {
			console.log("ELSE Ice Gathering State is now complete!");
		...
		resolve();
        	console.log("Inside ELSE of Promise");
            }
        });
    }).then(() => {
        console.log("Before sending the offer");
...
function start() {
    ...
    state.pc.onconnectionstatechange = (ev) => {
        console.log("Came in PC onconnectionstatechange()");
...
document.addEventListener('DOMContentLoaded', () => {
    console.log("Came inside addEL DOMContentLoaded")
    getcconnectionstatus()
    console.log("Came inside addEL DOMContentLoaded after getcconnectionstatus()")
    powerButton.onclick = () => {
        console.log("Came inside addEL DOMContentLoaded after onclick()")
        if(state.pc && state.pc.connectionState === "connected") {
            console.log("Came inside addEL DOMContentLoaded onClick in IF")
            stop()
            powerButton.classList.remove("text-danger")
            powerButton.classList.add("text-success")
        } else {
            console.log("Came inside addEL DOMContentLoaded onClick in ELSE")
            start()
